...
class PIIRedactor:
    ...
    def __init__(self, model_name: str = "en_core_web_sm"):
        ...
        self.generic_noun_stems = {
            ...
            # Common false positive stems
            'key', 'officers', 'independent', 'directors', 'statutory', 'auditors',
            'email', 'phone', 'contact', 'grievance', 'investor',
            # Indian city/location names (prevent as person names)
        ...
        self.patterns = {
            ...
            'phone': [
                # International formats with flexible digit grouping (10+ digit numbers)
                re.compile(r'(?:\+\s*\d{1,3}[\s-]?)(?:\d[\s-]?){7,12}'),
            ],
            'ip_address': [
            ...
            'reg_no': [
                ...
                # SEBI registration numbers (INM, INB, INP, INH, INA prefixed)
                re.compile(r'\bIN[MBPHA]\d{9,12}\b'),
            ],
            'dob': [
                re.compile(
            ...
            'company_name': [
                # Global corporate suffixes pattern — disallow periods to stop at sentence boundaries
                re.compile(r'\b([A-Z][a-zA-Z0-9\s,\&]{1,80}?(?:Private Limited|Pvt Ltd|Limited|Ltd|LLP|Inc|Corp|LLC|Corporation|Co\.|PLC|GmbH|AG|S\.A\.|Pty Ltd|Foundation|Association|Trust))\b', re.IGNORECASE)
            ]
        }
    ...
    def is_valid_person_name(self, name: str) -> bool:
        ...
        # Reject if it contains email addresses or URLs
            ...
            for pattern in patterns:
                for match in pattern.finditer(text):
                    ...
                    if matched_val and len(matched_val) > 2:
                        ...
                        if pii_type == 'date':
                            # Skip dates that are already captured as dob
                            if matched_val in results.get('dob', set()):
                                continue
